  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname.startsWith("/sub")) {
        return serveDashboard(APP_DOMAIN);
...
function serveDashboard(domain) {
  const uuid = crypto.randomUUID();
  const vlessLink = `vless://${uuid}@${domain}:443?encryption=none&security=tls&sni=${domain}&type=ws&host=${domain}&path=%2F#Nautica-Worker`;
  ...
  <head>
      ...
      <title>Nautica VLESS Proxy Node</title>
      <script src="https://cdn.tailwindcss.com"></script>
  </head>
  <body class="bg-gray-950 text-slate-200 min-h-screen flex items-center justify-center p-4">
      <div class="max-w-2xl w-full bg-slate-900 rounded-2xl shadow-[0_0_40px_rgba(79,70,229,0.15)] overflow-hidden border border-slate-800">
          <div class="p-6 md:p-8">
              <div class="flex items-center gap-3 mb-6">
                  ...
                  <div>
                      <h1 class="text-2xl font-bold text-white tracking-tight">VLESS Proxy Node</h1>
              ...
              <div class="space-y-6">
                  <div class="bg-slate-950 p-5 rounded-xl border border-slate-800">
                      <label class="block text-xs font-semibold text-slate-500 uppercase tracking-widest mb-3">Client Configuration (VLESS)</label>
                      <textarea class="w-full h-28 bg-slate-900 border border-slate-800 rounded-lg p-3 text-emerald-400 font-mono text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 resize-none transition-all" readonly id="vless-config">${vlessLink}</textarea>
                      <button onclick="copyConfig()" class="mt-4 w-full bg-indigo-600 hover:bg-indigo-500 text-white font-semibold py-3 rounded-lg transition-colors flex justify-center items-center gap-2 shadow-lg shadow-indigo-500/20">
                          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                          <span id="btn-text">Copy VLESS Link</span>
                      </button>
                  </div>

                  <div class="grid grid-cols-2 gap-4">
                      <div class="bg-slate-950 p-4 rounded-xl border border-slate-800">
                          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1.5">Generated UUID</label>
                          <p class="text-indigo-300 font-mono text-xs break-all truncate">${uuid}</p>
                      </div>
                      <div class="bg-slate-950 p-4 rounded-xl border border-slate-800">
                          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1.5">Worker Domain</label>
                          <p class="text-indigo-300 font-mono text-xs break-all truncate">${domain}</p>
                      </div>
                  </div>
              </div>
          </div>
          <div class="bg-black/40 p-4 text-center text-slate-600 text-xs border-t border-slate-800">
              Cloudflare Worker Edge Node &bull; High Performance Proxy Routing
          </div>
      </div>
      ...
      <script>
          function copyConfig() {
              ...
              btn.classList.remove('bg-indigo-600', 'hover:bg-indigo-500', 'shadow-indigo-500/20');
              setTimeout(() => {
                  textSpan.innerText = originalText;
                  btn.classList.remove('bg-emerald-600', 'hover:bg-emerald-500', 'shadow-emerald-500/20');
                  btn.classList.add('bg-indigo-600', 'hover:bg-indigo-500', 'shadow-indigo-500/20');
              }, 2000);
          }
      </script>
  </body>
